...
| `manager.js`                                                  | `DetachedGlassManager` class + its `detachedGlassManager` singleton: the registry + focus/stacking coordinator for **in-window** detached glasses. It owns _registration_, not the DOM or animation — `addDetachedGlass` (build + register + `bringToFront`, returns the element), `removeDetachedGlass` (unregister + splice, returns the element), `updateDetachedGlass` (stub), plus `bringToFront` / `getActiveDetachedGlass`. The DOM `append`/`remove` and the open/close animation live one layer up in `crud.js` / `windowless-glass.js`. Windowless glasses use their **own** manager — see §8.6 and the split below. |
...
A **windowless glass** is a detached glass with no owning window: it's appended to `document.body` instead of a `<bw-window>` and isn't bound to any `BinaryWindow` instance. It's registered with its **own** `windowlessGlassManager` (`binary-window/windowless-glass-manager.js`) — an empty subclass of `DetachedGlassManager` that inherits the whole registry + stacking/focus API but, via the inherited constructor, keeps its own `detachedGlassElements` array and `topZIndex`. So z-index/activation work the same _within_ the windowless set, but windowless and in-window detached glasses are **independent stacking/focus contexts**: a detached glass may sit above a windowless one and both kinds can hold `[active]` at once. Move/resize/activate work **without any mounted window** because those listeners are document-global and installed at module load (see "Standalone vs. per-instance features" in §8.5).

- **Create** (`binary-window/windowless-glass.js` `addWindowlessGlass`, a **static** method mixed onto `BinaryWindow` via `assembleStatic` — see §3) — calls `windowlessGlassManager.addDetachedGlass({ position: 'center', ... })` to build + register the glass, sets the `windowless` attribute, appends it to `document.body`, then (unless `animate: false`) plays the open animation. Returns a `Promise<Element>` that resolves once the open animation completes. **Remove** (`removeWindowlessGlass(id, { animate = true })`, also static) — `windowlessGlassManager.removeDetachedGlass(id)` to unregister, then `removeDetachedGlassElement` to animate out + remove the node and its modal backdrop (below). Also returns a `Promise<Element>`.
