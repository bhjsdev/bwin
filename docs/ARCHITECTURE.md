...
A sibling `assembleStatic(...modules)` mixes module objects onto the **class itself** (also via `strictAssign`) rather than the prototype, for methods that take no instance. It remains available but is currently unused: `addWindowlessGlass` / `removeWindowlessGlass` (`binary-window/windowless-glass.js`) are plain **module-level function exports** — they manage glasses with no owning window, so they're free functions rather than instance methods (see §8.6).
...
- **Pre-built `Sash` entry:** a `Sash` tree can be passed directly to `Frame`/`BinaryWindow` instead of a config object — `resolveConfig` branches on `settings instanceof Sash` and adopts it as `rootSash`. `Sash`'s constructor recursively rebuilds a plain sash-like tree (two `isSashLike` children) into real `Sash` instances, so a serialized layout (see `exportConfig`/`mergeConfig`, §9) round-trips back in through this path.
...
- detached-glass actions — `attach`, `minimize`, and `close` (for an **in-window** detached glass only). The detached **close** action on a _windowless_ glass routes through the module-level `removeWindowlessGlass` function (no instance), so it emits nothing.
...
| `crud.js`                                                     | `addDetachedGlass` / `removeDetachedGlass` (public) + `renderDetachedGlasses` (append config-restored glasses into `windowElement`). Cascades placement down-right from the active glass (`getCascadedPlacement`); guards size so the constructor's `222` debug default never fires.                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `detached-glass-manager.js`                                   | `DetachedGlassManager` class: the registry + focus/stacking coordinator for detached glasses. Each `BinaryWindow` builds its **own** instance in `resolveConfig` (`this.detachedGlassManager`), so windows have independent stacks; windowless glasses use a module-level instance (§8.6). It owns _registration_, not the DOM or animation — `addDetachedGlass` (build + register + `bringToFront`, returns the element), `removeDetachedGlass` (unregister + splice, returns the element), `updateDetachedGlass` (stub), `createConfig` (serialize the stack for export), plus `bringToFront` / `getActiveDetachedGlass`. The DOM `append`/`remove` and the open/close animation live one layer up in `crud.js` / `windowless-glass.js`. |
| `activate.js`                                                 | click-to-focus → `bringToFront`. **Per-instance**, delegated on `this.windowElement`; installed by `enableDetachedGlassFeatures` (windowless glasses have their own document-global activate in `windowless-glass.js`, §8.6).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `utils.js`                                                    | `generateMetricsByPosition`, resize-handle creation, `removeGlassBackdrop`, `getContainingBlockOrigin`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |

`DEFAULT_DETACHED_GLASS_ACTIONS = [minimize, attach, close]`. **minimize** (`action.minimize.js`) stashes the glass on a sill pot (`bwDetachedGlassElement`), then plays a FLIP-style flight — `animateElementToElement` (`src/animate.js`) shrinks/flies the glass onto its pot and resolves a promise; its `.then()` then sets the `[minimized]` attribute (hidden via CSS, and serializable for export — unlike inline `display:none`) and emits the `minimize` event (§7).

**Standalone vs. per-instance features.** `move`/`resize` attach **document-global, instance-independent** listeners (they find their target via `closest('bw-glass[detached]')` and never read `this`). They're installed by `enableDetachedGlassStandaloneFeatures()`, called **once at module load** in `binary-window.js` (module evaluation is one-time, so no idempotency flag is needed). This is what makes the `addWindowlessGlass` path work with **no mounted window** — importing `BinaryWindow` is enough to wire move/resize. **Activate is now per-instance**, not standalone: `activate.js`'s `enableDetachedGlassActivate` is delegated on `this.windowElement` and dispatches `bringToFront` to `this.detachedGlassManager`, so it dies with a rebuilt window (via `reframe`) instead of leaking the dead instance and racing the live one on the `[active]` guard. Windowless glasses get their own document-global activate (`enableWindowlessGlassActivate` in `windowless-glass.js`, run once from `src/index.js`) targeting `bw-glass[detached][windowless]` and its module-level manager, so the two stacks stay independent (§8.6). Un-potting (restore from the sill) is a per-instance sill feature wired by `enableSillFeatures()` (see `sill.js`), which needs `this.sillElement` (windowless glasses have no sill). _Historical note:_ an earlier `drag.js` offered native-DnD repositioning (docked to panes) as an alternative to `move.js`; it was removed in favor of free-floating `move.js`.

**Manager owns _registration_; the layer above owns DOM + animation.** A `DetachedGlassManager` is a registry only — the DOM `append`/`remove` and the open/close animation live in the `crud.js` (in-window, wrapping the window's own `this.detachedGlassManager`) and `windowless-glass.js` (windowless, wrapping the module-level `windowlessGlassManager`) layers, which both return a `Promise<Element>`. Both are instances of the same class, so the method contract below is identical for each:
...
A **windowless glass** is a detached glass with no owning window: it's appended to `document.body` instead of a `<bw-window>` and isn't bound to any `BinaryWindow` instance. It's registered with its **own** module-level `windowlessGlassManager` — a plain `new DetachedGlassManager()` in `binary-window/windowless-glass.js`, with its own `detachedGlassElements` array and `topZIndex`. So z-index/activation work the same _within_ the windowless set, but windowless and in-window detached glasses are **independent stacking/focus contexts**: a detached glass may sit above a windowless one and both kinds can hold `[active]` at once. Move/resize work **without any mounted window** because those listeners are document-global and installed at module load; windowless activate is its own document-global listener run once from `src/index.js` (see "Standalone vs. per-instance features" in §8.5).

- **Create** (`binary-window/windowless-glass.js` `addWindowlessGlass`, a **module-level function export** — see §3 and §9) — calls `windowlessGlassManager.addDetachedGlass({ position: 'center', ... })` to build + register the glass, sets the `windowless` attribute, appends it to `document.body`, then (unless `animate: false`) plays the open animation. Returns a `Promise<Element>` that resolves once the open animation completes. **Remove** (`removeWindowlessGlass(id, { animate = true })`, also a function export) — `windowlessGlassManager.removeDetachedGlass(id)` to unregister, then `removeDetachedGlassElement` to animate out + remove the node and its modal backdrop (below). Also returns a `Promise<Element>`.
...
- **Modal z-index reservation** (`manager.js`, inherited base behavior) — `bringToFront` increments `topZIndex` by **2** (not 1) and returns the new value, reserving the odd slot just below for a modal overlay on a windowless glass. Windowless glasses inherit this from `DetachedGlassManager`, so the reserved slot is available in the windowless stack.
...
Exports: `Frame`, `BinaryWindow`, `Sash`, `ConfigRoot`, `mergeConfig`, `Position`, `addWindowlessGlass`, `removeWindowlessGlass`, `DEFAULT_GLASS_ACTIONS`, `DEFAULT_DETACHED_GLASS_ACTIONS`, `DEFAULT_WINDOWLESS_GLASS_ACTIONS`, and the **deprecated** `BUILTIN_ACTIONS` (backwards-compat alias). Importing the module also runs `enableWindowlessGlassFeatures()` once (wires the windowless activate listener). CSS is imported here (`vars/body/frame/glass/detached-glass/sill`) and shipped as `bwin.css`.
...
- `exportConfig()` — serialize the live window to a plain, JSON-friendly object: the trimmed sash tree (`trimSashLikeTree`, no `store`) plus `fitContainer`/`theme`/`actions`, the minimized `pots` (`createPotConfig`), and the detached-glass stack (`detachedGlassManager.createConfig`). `reframe(settings)` rebuilds a mounted window in place (`deglaze` → `resolveConfig` → `glaze`). Round-trips back in via the config constructor or `mergeConfig`.
- `setTheme(theme)` / `fit()`.
- `addWindowlessGlass(options)` / `removeWindowlessGlass(id, { animate })` — **module-level function exports** (not instance/static methods) for floating panels on `document.body` with no owning window; both return a `Promise<Element>` (§8.6).
- `mergeConfig(sashLikeObject, configRootLikeObject)` — a **module-level export** that merges a serialized layout (an `exportConfig` result) with a `ConfigRoot`-like object: it re-seeds each node's `store` (glass props) from the config by matching sash id, and carries over `fitContainer`/`theme`/`actions`/`pots`/`detachedGlasses`. Returns a `Sash` tree ready to pass to `new BinaryWindow(...)`.
